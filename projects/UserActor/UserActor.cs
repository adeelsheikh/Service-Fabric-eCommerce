...
namespace UserActor
{
    ...
    internal class UserActor : Actor, IUserActor
    {
        ...
        {
            foreach (var productId in await StateManager.GetStateNamesAsync())
            {
                await StateManager.RemoveStateAsync(productId);
            }
        }

        public async Task<Dictionary<Guid, int>> GetCartItems()
        {
            ...
            {
                if (Guid.TryParse(stateName, out Guid productId))
                {
                    int quantity = await StateManager.GetStateAsync<int>(stateName);
